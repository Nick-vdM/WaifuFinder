...
def calculateRankings(faceData, mainFace):
    ...
    for face in faceData:
        if index % 100 == 0:
            print("Scanning Waifus...", index, "out of", len(faceData))
        index += 1
...
def saveAllAnimeFaces(path, fileToSaveTo):
    ...
    for f in files:
        if processedFaces % onePercent == 0:
            print(str(processedFaces), "faces complete")
...
def displayImages(mainImagePath, imagePathsList):
    ...
    f, spots = plt.subplots(1, 1 + len(imagePathsList), figsize=(200, 100))
    image = cv2.cvtColor(cv2.imread(mainImagePath), cv2.COLOR_BGR2RGB)
...
if __name__ == "__main__":
    # saveAllAnimeFaces("data/preCroppedFaces", "data/waifuFaces.pickle")
    # mainFace = faceInformation()
    # mainFace.processHumanFace("data/asianMale.jpg", 1)
    # faceData = loadFaceInformation('data/waifuFaces.pickle')
    # topFiles = searchKClosestFace(mainFace, faceData)
    # displayImages("data/asianMale.jpg", topFiles)
